...
class SSH {
	...
	putFiles(files, options = {}) {
		...
		for(let i = 0, len = files.length; i < len; i++) {
			let file = files[i];
			if (typeof file.localPath !== 'string' || typeof file.remotePath !== 'string') {
				throw console.log('putFiles: localPath | remotePath must be an object');
			}
		}

		let sftp = this.requestSFTP();
		return Promise.all(files.map(file => this.putFile(file.localPath, file.remotePath, options, sftp)));
	}
